                                ...
                                cuint batch_size, cuint M, cuint N, cuint K)
{
    ...
    extern __shared__ half shmem_half[][HALF_K_TILE_CHUNK_PADDED_LEN];
    ...
    float *shmem_ptr_c_warp = (float *)&shmem_half[0][0] +
...
#pragma unroll
            ...
            {
                *((int4 *)&shmem_half[shmem_idx_for_ab][0] + i) = *gmem_ab_ptr;
...
#pragma unroll
                for (int i = 0; i < 2; i++)
                {
                    ...
                    const half *tile_ptr = &shmem_half[shmem_idx_a][k_step * 16];
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        if (i == 0)
                        {
                            ...
                            size_t shmem_idx_b = 128 +
                                                 (4 * 16) * (warpId % 2) +
                                                 (j * 16);
                            const half *tile_ptr = &shmem_half[shmem_idx_b][k_step * 16];
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, py_module)
{
    py_module.def("bmm_half", &bmm_half, "Matrix multiplication for half.")
        .def("bmm_uint8", &bmm_uint8, "Matrix multiplication for uint8.");
}
