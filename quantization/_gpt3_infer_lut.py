    ...
    def forward(self, hidden_states, set_inference_key_value_memory=False, inference_max_sequence_len=None):
        ...
        if inference_max_sequence_len:
            ...
            if self.quant_kmax is None:
                ...
                self.quant_ktable = torch.tensor(
                    [i * self.quant_kscaling + self.quant_kmin for i in range(16)],
                    dtype=torch.float16,
                ...
                self.quant_vtable = torch.tensor(
                    [i * self.quant_vscaling + self.quant_vmin for i in range(16)],
                    dtype=torch.float16,
            ...
            k1 = key_layer >> 4
            k2 = key_layer << 4 >> 4
            key_layer = torch.concat([k1, k2], dim=-1).to(torch.int)
            key_layer = self.quant_ktable[key_layer]
            v1 = value_layer >> 4
            v2 = value_layer << 4 >> 4
            value_layer = torch.concat([v1, v2], dim=-1).to(torch.int)
            value_layer = self.quant_vtable[value_layer]
            torch.cuda.synchronize()
            d_end = time.time()
            print(1000 * (d_end - d_start))
