    ...
    def forward(self, hidden_states, set_inference_key_value_memory=False, inference_max_sequence_len=None):
        ...
        if inference_max_sequence_len:
            ...
            if self.quant_kmax is None:
                self.quant_kmax = torch.max(key_layer, dim=3, keepdim=True).values
                self.quant_kmin = torch.min(key_layer, dim=3, keepdim=True).values
                self.quant_kscaling = (self.quant_kmax - self.quant_kmin) / 16
                self.quant_vmax = torch.max(value_layer, dim=3, keepdim=True).values
                self.quant_vmin = torch.min(value_layer, dim=3, keepdim=True).values
...
class GPTModel(torch.nn.Module):
    ...
    def forward(self, inputs, inf_target_tensor):
