...
from _gpt3_infer_quant_perp import GPTModel, GPTTokenizer
...
def sample_sequence_batch(
    ...
    with torch.no_grad():
        ...
        while infer_cursor < max_len:
            ...
            output = model(model_inputs, INF_TARGET_TENSOR)
...
def gpt_generate(inputs: List[str], max_gen_len):
    ...
    target_tensor = token_pool[:, 1:2049]
    target_tensor = target_tensor[0, :]
    global INF_TARGET_TENSOR
    INF_TARGET_TENSOR = target_tensor
    token_pool = token_pool[:, :2049]
    prompt_lengths = torch.tensor([2048], dtype=torch.int64, device=prompt_lengths.device)
