...
class Embedding(torch.nn.Module):
    ...
    def forward(self, input_ids, position_ids):
        ...
        for i in range(seq_len // CONTEXT_LEN):
            indices.append((i + 1) * CONTEXT_LEN)
        position_ids = torch.tensor_split(position_ids, indices, dim=1)
        if position_ids[-1].size(1) == 0 and len(position_ids) > 1:
            position_ids.pop()
        pos_emb_list = []
        for pos_id in position_ids:
            position_embeddings = self.position_embeddings(pos_id)
            pos_emb_list.append(position_embeddings)
        position_embeddings = torch.concat(pos_emb_list, dim=1)
...
def _attention_mask_func(attention_scores, attention_mask):
...
class _CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer):
        ...
        attention_probs = self.scale_mask_softmax(attention_scores, _attention_mask)
...
class Attention(torch.nn.Module):
    ...
    def forward(self, hidden_states, set_inference_key_value_memory=False, inference_max_sequence_len=None):
        ...
        if inference_max_sequence_len:
            ...
            # Quantize
            if self.quant_kmax is None:
                self.quant_kmax = torch.max(key_layer)
                self.quant_kmin = torch.min(key_layer)
                self.quant_kscaling = (self.quant_kmax - self.quant_kmin) / 16
                self.quant_vmax = torch.max(value_layer)
                self.quant_vmin = torch.min(value_layer)
                self.quant_vscaling = (self.quant_vmax - self.quant_vmin) / 16
            key_layer = ((key_layer - self.quant_kmin + self.quant_kscaling / 2) / self.quant_kscaling).to(torch.uint8)
            k1 = key_layer[..., 0:64] << 4
            k2 = key_layer[..., 64:128]
            key_layer = k1 + k2
            value_layer = ((value_layer - self.quant_vmin + self.quant_vscaling / 2) / self.quant_vscaling).to(torch.uint8)
            ...
            key_layer = (key_layer * self.quant_kscaling + self.quant_kmin).to(torch.float16)
            v1 = value_layer >> 4
            v2 = value_layer << 4 >> 4
            value_layer = torch.concat([v1, v2], dim=-1)
            value_layer = (value_layer * self.quant_vscaling + self.quant_vmin).to(torch.float16)
            torch.cuda.synchronize()
            d_end = time.time()
            print(f"Dequant: {1000 * (d_end - d_start)}")
...
class TransformerLayer(torch.nn.Module):
    ...
    def forward(self, hidden_states, set_inference_key_value_memory=False, inference_max_sequence_len=None):
        ...
        torch.cuda.synchronize()
        a_start = time.time()
        print(f"Layernorm1: {1000 * (a_start - l_start)}")
        ...
        torch.cuda.synchronize()
        a_end = time.time()
        print(f"Attn: {1000 * (a_end - a_start)}")
        ...
        torch.cuda.synchronize()
        d1_end = time.time()
        print(f"Dropout1: {1000 * (d1_end - a_end)}")
        ...
        torch.cuda.synchronize()
        ln2_end = time.time()
        print(f"Layernorm2: {1000 * (ln2_end - d1_end)}")
        ...
        torch.cuda.synchronize()
        mlp_end = time.time()
        print(f"MLP: {1000 * (mlp_end - ln2_end)}")
        ...
        torch.cuda.synchronize()
        d2_end = time.time()
        print(f"Dropout2: {1000 * (d2_end - mlp_end)}")

        torch.cuda.synchronize()
        l_end = time.time()
        print(f"Layer: {1000 * (l_end - l_start)}")
