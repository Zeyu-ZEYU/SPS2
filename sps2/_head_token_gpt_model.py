    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        if self.layer_num > 0:
            ...
            for head in range(16):
                tensor = attention_scores[0, head, :, :]
                tensor = torch.mean(tensor, dim=1)
                ids = [(i, v.item()) for i, v in enumerate(tensor)]
                ids.sort(key=lambda x: x[1])
                # ids = ids[: int(ratio * len(ids))]
                ids = ids[int(2048 * 0.7) :]
                result_data[head] = set([id for id, _ in ids])
            for a in range(16):
                line_str = ""
                for b in range(a, 16):
                    set1: set = result_data[a]
                    set2: set = result_data[b]
